  ...
  const uploadToBackend = async (file: File) => {
    ...
    try {
      ...
      if (response.ok && data.id) {
        ...
        throw new Error(data.error || "Upload failed: Invalid response data")
      }
    } catch (error) {
      console.error("Upload error:", error)
      alert(`File upload failed: ${error instanceof Error ? error.message : "Network error or invalid server response."}`)
    } finally {
